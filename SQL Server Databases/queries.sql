-- Request to create an index that makes the query below run faster
...
-- Request to show entries' title whose exhibit's title contains word 'πληροφορική' using index(es) that (maybe) accelerates query's performance
CREATE INDEX index_title ON bibrecs(title)
...
-- Request to show entries' title and material whose exhibit's title is 'Economics' using index(es) that (maybe) accelerates query's performance
CREATE INDEX index_title ON bibrecs(title)
...
-- Request to show entries' title and material whose exhibit's title starts with 'Economics' using index(es) that (maybe) accelerates query's performance
CREATE INDEX index_title ON bibrecs(title)
...
-- Request to show entries' title and language whose publisher is 'Κλειδάριθμος' using index(es) that (maybe) accelerates query's performance
...
-- Request to show for each department the total number of loans in 2000 using index(es) that (maybe) accelerates query's performance
...
-- Request to show entries' title, language and author's name whore subject term is 'Databases' using index(es) that (maybe) accelerates query's performance
...
-- Request to show (with 2-3 different ways) entries' id and book's title for which library has at least one copy at location 'OPA' and at least one copy at location 'ANA'. Fourth query has the best performance, so for this query it's requested to accelerate its performance using index(es).
